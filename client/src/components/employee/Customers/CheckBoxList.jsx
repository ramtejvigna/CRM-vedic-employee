...
export const handleSendMail = async (pdfUrl, uniqueId, email) => {
  ...
  try {
    ...
    await axios.post(
      "https://vedic-backend-neon.vercel.app/api/send-pdf-email",
      {
        email,
        base64Pdf,
        uniqueId,
...
export const handleSendWhatsApp = async (pdfUrl, uniqueId, phoneNumber) => {
  ...
  try {
    ...
    const res = await axios.post(
      "https://vedic-backend-neon.vercel.app/api/send-pdf-whatsapp",
      {
        phoneNumber,
        base64Pdf,
        uniqueId,
...
const CheckBoxListPage = () => {
    ...
    const fetchEmployee = async () => {
      try {
        ...
        const response = await axios.get(
          `https://vedic-backend-neon.vercel.app/api/employees/get-employee?id=${id}`
        );
        setEmployee(response.data.employee);
      } catch (err) {
        setError(
          err.response ? err.response.data : "Error fetching employee details"
        );
      }
    };
  ...
  const handleNumberOfNamesChange = (e) => {
    ...
    setAdditionalBabyNames(
      Array.from({ length: count }, () => ({ nameEnglish: "", meaning: "" }))
    );
  };
      ...
      <div className="overflow-hidden flex items-center justify-center mt-4">
        {dots
          ...
          .map((dot, index) => (
            <span
              key={index}
              className="transition-opacity duration-300 ease-in-out mx-1"
            >
              {dot}
            </span>
          ))}
    ...
    axios
      ...
      .then((response) => {
        ...
        const extractUniqueValues = (field) =>
          [
            ...new Set(allNames.map((item) => item[field]).filter(Boolean)),
    ...
    const filtered = Object.keys(filters).reduce((result, key) => {
      ...
      if (key === "startingLetter") {
        return result.filter(
          (item) =>
            item.nameEnglish.charAt(0).toLowerCase() ===
      ...
      if (key === "gender") {
        ...
        return result.filter(
          (item) => item.gender?.toLowerCase() === filters[key].toLowerCase()
        );
      }
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://vedic-backend-neon.vercel.app/api/create-pdf`,
        {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleFilterChange("startingLetter", e.target.value)
                }
                maxLength={1}
                className="mt-1 p-2 block w-full border rounded-md"
              />
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  handleFilterChange("nakshatra", e.target.value)
                }
                className="mt-1 p-2 block w-full border rounded-md"
              >
                  ...
                  {numberOfNames > 0 && (
                    <div className="relative">
                      ...
                      <button
                        ...
                        className={`absolute left-0 top-1/2 transform -translate-y-1/2 p-2 rounded-full bg-gray-200 ${
                          currentStep === 0
                      ...
                      <Slider
                        {...settings}
                        ref={sliderRef}
                        className="mb-4 ml-11 w-full max-w-[calc(100%-100px)] relative"
                      >
                        {" "}
                      ...
                      <button
                        ...
                        className={`absolute right-0 top-1/2 transform -translate-y-1/2 p-2 rounded-full bg-gray-200 ${
                      ...
                      {currentStep === numberOfNames - 1 && ( // Only show button on last step
                        <button
                          onClick={() => {
                            const allFilled = additionalBabyNames.every(
                              (entry) => entry.nameEnglish && entry.meaning
                            );
                            if (!allFilled) {
                              toast.error(
