...
const BabyDatabase = () => {
  ...
  const fetchBabyNames = async () => {
    ...
    try {
      const response = await axios.get(
        "https://vedic-backend-neon.vercel.app/api/names"
      );
  ...
  const requestedAccess = async () => {
    try {
      const response = await axios.get(
        "https://vedic-backend-neon.vercel.app/api/employees/requestBabyNames",
        {
          params: { employeeId },
        }
      );
  ...
  const checkAcceptance = async () => {
    try {
      const response = await axios.get(
        `https://vedic-backend-neon.vercel.app/api/employees/adminAcceptance`,
        {
  ...
  const filteredNames = babyNames.filter((baby) => {
    ...
    const matchesStartingLetter =
      !startingLetterFilter ||
      baby.nameEnglish
        ?.toLowerCase()
        .startsWith(startingLetterFilter.toLowerCase());

    const matchesZodiac =
      !selectedFilters.zodiac ||
      baby.zodiac?.toLowerCase() === selectedFilters.zodiac.toLowerCase();

    const matchesNakshatra =
      !selectedFilters.nakshatra ||
    ...
    const matchesBookName =
      !selectedFilters.bookName ||
      baby.bookName?.toLowerCase() === selectedFilters.bookName.toLowerCase();

    const matchesPlanetary =
      !selectedFilters.planetaryInfluence ||
      baby.planetaryInfluence?.toLowerCase() ===
        selectedFilters.planetaryInfluence.toLowerCase();

    const matchesRelatedFestival =
      !selectedFilters.relatedFestival ||
      baby.relatedFestival?.toLowerCase() ===
        selectedFilters.relatedFestival.toLowerCase();
  ...
  const handleAddName = async (newName) => {
    try {
      await axios.post("https://vedic-backend-neon.vercel.app/api/names", {
        newName: newName,
  ...
  const handleExcelUpload = async (event) => {
    ...
    try {
      await axios.post(
        "https://vedic-backend-neon.vercel.app/uploadExcelNames",
        formData,
        {
  ...
  const saveEdit = async () => {
    try {
      await axios.put(
        `https://vedic-backend-neon.vercel.app/updateBabyName/${editingName._id}`,
        editingName
      );
      setEditingName(null);
      fetchBabyNames();
    ...
    <div className="p-4 md:p-8 min-h-screen">
      ...
      <div className="mb-6 space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-10">
          ...
          <motion.button
            ...
            className={`px-4 py-2 rounded-lg text-white transition duration-300 flex items-center justify-center ${
              !checkAdminAcceptance
                ? "bg-gray-400 cursor-not-allowed"
      ...
      <div className="overflow-x-auto">
        ...
        >
                        ...
                        >
                          {editingName && editingName._id === baby._id ? (
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                <input
                                  ...
                                  onChange={(e) =>
                                    setEditingName({
                                      ...editingName,
                                      characterSignificance: e.target.value,
                                    })
                                  }
                                  className="w-full border border-gray-300 rounded-md px-2 py-1"
                                />
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                <input
                                  ...
                                  onChange={(e) =>
                                    setEditingName({
                                      ...editingName,
                                      relatedFestival: e.target.value,
                                    })
                                  }
                                  className="w-full border border-gray-300 rounded-md px-2 py-1"
                                />
                            ...
                            <>
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                {baby.characterSignificance}
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap">
                                {baby.mantraRef}
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap">
                                {baby.relatedFestival}
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap">
                                {baby.extraNote}
                              </td>
          ...
          {filteredNames.length === 0 && !loading && <EmptyBabyNames />}
        </motion.div>
      </div>
      ...
      {!loading && (
        <div className="mt-4 flex flex-col sm:flex-row justify-between items-center gap-4 rounded-lg px-4 py-3">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm">{`Page ${page + 1} of ${Math.ceil(
              filteredNames.length / rowsPerPage
            )}`}</span>
            <button
              onClick={() => handleChangePage(page + 1)}
              disabled={
                page >= Math.ceil(filteredNames.length / rowsPerPage) - 1
              }
              className="px-3 py-1 rounded bg-indigo-100 text-indigo-700 disabled:opacity-50 transition duration-300 text-sm"
            >
              Next
